<template>
	<div class="misty-select">
		...
		<div class="misty-select-content">
			<select v-model="defaultValue" @change="selectChange" :class="{'misty-select-content-srtl':direction==='rtl','misty-select-content-sltr':direction==='ltr'}">
				<option disabled selected value="">{{placeholder}}</option>
				<template v-for="item in configure">
					<option :value="item[valueMap[0]]">{{item[valueMap[1]]}}</option>
				</template>
			</select>
			<i class="misty-select-content-clear" @click="selectClear" v-if="defaultValue"></i>
		</div>
	</div>
</template>
