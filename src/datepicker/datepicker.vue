<template>
	<div>
		{{formattedValue}}
		<s-popover class="datepicker-popover" placement="bottom" trigger="click" @open="showPanel" ref="popover">
				...
				<div class="date-panel-body">
					<div v-if="model === 'day'">
						...
						<div class="date-panel-date" v-for="(item1, i) in helper.getRange(1,6)" :key="i">
							<span v-for="(item2, j) in helper.getRange(1,7)" :key="j" @click="selectDate(getVisibleDate (i, j))" class="date-cell"
					...
					<div v-show="model === 'year'">
						<div class="year-wrapper" ref="yearWrapper">
							<div v-for="(item, index) in yearsList" :key="index" @click="selectYear(item)" class="year-item" :class="getYearClass(item)">{{item}}</div>
						</div>
					</div>
					<!-- month -->
					<div v-show="model==='month'">
						<div class="month-wrapper" ref="monthWrapper">
							<div v-for="(item, index) in monthInAYear" :key="index" @click="selectMonth(index)" class="month-item" :class="getMonthClass(index)">{{item}}月</div>
						</div>
					</div>
				</div>
		...
		methods: {
			...
			resetDisplayInfo() {
				...
				let {
					year,
					month
				} = helper.getDateInfo(displayDateObj)
